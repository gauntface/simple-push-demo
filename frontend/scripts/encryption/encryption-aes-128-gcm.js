...
export class EncryptionAES128GCM {
	...
	async encryptPayload(subscription, payloadText) {
		...
		return {
			...
			publicServerKey: uint8ArrayToBase64Url(exportedServerKeys.publicKey),
		};
	}
	...
	_generateCEKInfo(subscription, serverKeys) {
		const utf8Encoder = new TextEncoder('utf-8');
		const contentEncoding8Array = utf8Encoder.encode(
			'Content-Encoding: aes128gcm',
		);
		const paddingUnit8Array = new Uint8Array(1).fill(0);
		return joinUint8Arrays([contentEncoding8Array, paddingUnit8Array]);
	}
	...
	async _generatePRK(subscription, serverKeys) {
		...
		const hkdf = new HKDF(sharedSecret, subscription.getKey('auth'));
		return hkdf.generate(keyInfoUint8Array, 32);
	}

	async _getSharedSecret(subscription, serverKeys) {
		const keys = await arrayBuffersToCryptoKeys(subscription.getKey('p256dh'));
