...
} from '/frontend/scripts/encryption/helpers.js';
import { EncryptionAESGCM } from
...
describe('EncryptionAESGCM', function() {
	...
	const VALID_SERVER_KEYS = {
		publicKey:
			'BG3OGHrl3YJ5PHpl0GSqtAAlUPnx1LvwQvFMIc68vhJU6nIkRzPEqtCduQz8wQj0r71' +
			'NVPzr7ZRk2f-fhsQ5pK8',
	...
	const VALID_SUBSCRIPTION = {
		endpoint:
			'https://android.googleapis.com/gcm/send/FAKE_GCM_REGISTRATION_ID',
		getKey: (keyId) => {
			switch (keyId) {
			case 'p256dh':
				return base64UrlToUint8Array(
					'BCIWgsnyXDv1VkhqL2P7YRBvdeuDnlwAPT2guNhdIoW3IP7GmHh1SMKPLxRf7x8' +
					'vJy6ZFK3ol2ohgn_-0yP7QQA=',
				);
			case 'auth':
				return base64UrlToUint8Array('8eDyX_uCN0XRhSbY5hs7Hg==');
			default:
				throw new Error(
					'Oh dear. An unknown subscription key was requested: ',
					keyId,
				);
			}
		},
	...
	const VALID_OUTPUT = {
		...
		context:
			'UC0yNTYAAEEEIhaCyfJcO_VWSGovY_thEG9164OeXAA9PaC42F0ihbcg_saYeHVIwo8' +
			'vFF_vHy8nLpkUreiXaiGCf_7TI_tBAABBBG3OGHrl3YJ5PHpl0GSqtAAlUPnx1LvwQv' +
			'FMIc68vhJU6nIkRzPEqtCduQz8wQj0r71NVPzr7ZRk2f-fhsQ5pK8',
		cekInfo:
			'Q29udGVudC1FbmNvZGluZzogYWVzZ2NtAFAtMjU2AABBBCIWgsnyXDv1VkhqL2P7YRB' +
			'vdeuDnlwAPT2guNhdIoW3IP7GmHh1SMKPLxRf7x8vJy6ZFK3ol2ohgn_-0yP7QQAAQQ' +
			'Rtzhh65d2CeTx6ZdBkqrQAJVD58dS78ELxTCHOvL4SVOpyJEczxKrQnbkM_MEI9K-9T' +
			'VT86-2UZNn_n4bEOaSv',
		nonceInfo:
			'Q29udGVudC1FbmNvZGluZzogbm9uY2UAUC0yNTYAAEEEIhaCyfJcO_VWSGovY_thEG9' +
			'164OeXAA9PaC42F0ihbcg_saYeHVIwo8vFF_vHy8nLpkUreiXaiGCf_7TI_tBAABBBG' +
			'3OGHrl3YJ5PHpl0GSqtAAlUPnx1LvwQvFMIc68vhJU6nIkRzPEqtCduQz8wQj0r71NV' +
			'Pzr7ZRk2f-fhsQ5pK8',
	...
	const VALID_VAPID_KEYS = {
		publicKey: 'BG3OGHrl3YJ5PHpl0GSqtAAlUPnx1LvwQvFMIc68vhJU6nIkRzPEqtCdu' +
			'Qz8wQj0r71NVPzr7ZRk2f-fhsQ5pK8',
		privateKey: 'Dt1CLgQlkiaA-tmCkATyKZeoF1-Gtw1-gdEP6pOCqj4',
	};
	...
	it('should calculate a shared secret', async () => {
		...
		const serverKeys = await encryptionHelper.getServerKeys();
		const sharedSecret = await encryptionHelper._getSharedSecret(
			VALID_SUBSCRIPTION,
			serverKeys,
		);
	...
	it('should generate a context', async () => {
		const encryptionHelper = new EncryptionAESGCM();
		const serverKeys = await encryptionHelper.getServerKeys();
		const context = await encryptionHelper._generateContext(
			VALID_SUBSCRIPTION,
			serverKeys,
		);
	...
	it('should generate a cekInfo for aesgcm', async () => {
		...
		const serverKeys = await encryptionHelper.getServerKeys();
		const cekInfo = await encryptionHelper._generateCEKInfo(
			VALID_SUBSCRIPTION,
			serverKeys,
		);
	...
	it('should generate a nonceInfo with a context', async () => {
		...
		const serverKeys = await encryptionHelper.getServerKeys();
		const nonceInfo = await encryptionHelper._generateNonceInfo(
			VALID_SUBSCRIPTION,
			serverKeys,
		);
	...
	it('should generate a pseudo random key for aesgcm', async () => {
		...
		const serverKeys = await encryptionHelper.getServerKeys();
		const prk = await encryptionHelper._generatePRK(
			VALID_SUBSCRIPTION,
			serverKeys,
		);
	...
	it('should generate encryption keys', async () => {
		...
		const serverKeys = await encryptionHelper.getServerKeys();
		const keys = await encryptionHelper._generateEncryptionKeys(
			VALID_SUBSCRIPTION,
	...
	it('should generate the specific encryption keys', async () => {
		...
		const base64cek = uint8ArrayToBase64Url(
			new Uint8Array(keys.contentEncryptionKey),
		);
	...
	it('should encrypt message', async () => {
		const encryptionHelper = new EncryptionAESGCM();
		const encryptedPayload = await encryptionHelper.encryptPayload(
			VALID_SUBSCRIPTION,
			PAYLOAD,
		);
	...
	it('should encrypt message to a specific value', async () => {
		...
		const base64EncryptedPayload = uint8ArrayToBase64Url(
