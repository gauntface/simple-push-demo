		...
		describe(`Perform Tests in ${browserInfo.getPrettyName()}`, function() {
			...
			async function initDriver() {
				...
				case 'firefox': {
					...
					// https://bugzilla.mozilla.org/show_bug.cgi?id=1275521
					// Unfortunately it doesn't seem to work :(
					const ffOpts = new seleniumFirefox.Options();
					ffOpts.setPreference(
				...
				case 'opera': {
					...
					operaPreferences.profile.content_settings.exceptions.notifications[
						testServerURL + ',*'
					] = {
					...
					fs.writeFileSync(
						`${tempPreferenceFile}/Preferences`,
						JSON.stringify(operaPreferences),
					);
				...
				try {
					...
					if (
						browserInfo.getId() === 'firefox' &&
						browserInfo.getVersionNumber() === 56
					) {
						// See: https://github.com/mozilla/geckodriver/issues/800
						console.warn(
							'Swallowing setScriptTimeoutError() <- Geckodriver issue.',
						);
					} else {
						throw err;
					}
			...
			it(`pass sanity checks and trigger and receive a tickle`, async () => {
				...
				const performanceEntriesString =
				...
				performanceEntries.forEach((entry) => {
					requiredFiles.forEach((requiredFile) => {
						if (
							entry.name.indexOf(requiredFile) ===
							entry.name.length - requiredFile.length
						) {
				...
				if (requiredFiles.length !== 0) {
					throw new Error(
						'Missing required files in the final page',
						requiredFiles,
					);
				}
				...
				const notificationInfo = await getNotificationInfo(
					globalDriverReference,
				);

				notificationInfo.length.should.equal(1);
				notificationInfo[0].title.should.equal('Hello');
				notificationInfo[0].body.should.equal(
					'Thanks for sending this push msg.',
				);
				...
				notificationInfo[0].icon
					.indexOf(notifcationImg)
					.should.equal(
						notificationInfo[0].icon.length - notifcationImg.length,
					);
			});

			it(`trigger and receive a tickle via CURL`, async () => {
				// Load simple push demo page
				await initDriver();
				...
				const curlCommand = await globalDriverReference.executeScript(
					function() {
						const curlCodeElement = document.querySelector('.js-curl-code');
			...
			it(`enter payload text and receive a push message in ` +
				`${browserInfo.getPrettyName()}`, async () => {
				// Load simple push demo page
				await initDriver();
			...
			it(`trigger and receive a message with payload via CURL`, async () => {
				// Load simple push demo page
				await initDriver();
